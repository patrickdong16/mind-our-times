...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.8 | 2026-02-07 | **流程重构**：cron 改为 `mot-full-workflow.py`（一体化：生成→质检→发布→推荐）；H5 投票废弃，改微信原生投票；深度长文字数改为 ~2000 字 |
| v1.7 | 2026-02-07 | **防退步加固**：mot-content-generator prompt 强制提取真实作者名+新增 `extract_author_from_url()` 页面 meta 抓取；mot-quality-check 禁用词扩充（"本文来源于"等）；generate_recommendations 对齐结构化格式 |
